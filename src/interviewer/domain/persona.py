...
class SpeechStyle(BaseModel):
    ...
    def describe(self) -> list[str]:
        lines = [
            "说话长度：" + _scale(
                self.verbosity,
                ("一句话说完，绝不铺垫", "两三句话，直给", "会补充背景和例子", "喜欢长篇展开，但仍会把问题问清"),
            ),
            "态度温度：" + _scale(
                self.warmth,
                ("冷硬，几乎不给情绪回应", "克制，偶尔点头式回应", "友善，会肯定对方亮点", "非常热情，主动鼓励"),
            ),
            "用语正式度：" + _scale(
                self.formality,
                ("口语化，可以带口头禅", "自然口语，偶有俚语", "偏书面，术语准确", "高度正式，措辞严谨"),
            ),
            "语速：" + _scale(
                self.speech_rate,
                ("刻意放慢，留白多", "平稳", "偏快，节奏紧", "极快，一句接一句"),
            ),
        ]
    ...
    return [
        ...
        PersonaContract(
            ...
            speech=SpeechStyle(
                ...
                formality=6,
                speech_rate=4,
                catchphrases=["我理解", "能再具体一点吗", "谢谢你的分享"],
                banned_phrases=["你这个不行"],
            ),
            pressure=PressureProfile(
                aggression=1,
